...
void video_set_resolution(u32 xres, u32 yres, u32 bpp) {
    ...
    u64 ms_start = timer_get_ticks() / 1000; // get ticks ms before drawing on screen.
    for (int y=0; y<yres; y++) {
        if (y == 40) {
            // Redraw the "Resolution" string since it's been overwritten by the square.
