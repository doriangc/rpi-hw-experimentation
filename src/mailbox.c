...
static void mailbox_write(u8 channel, u32 data) {
    ...
    MBX()->write = (data & 0xFFFFFFFF | (channel & 0xF));
}
...
u32 mailbox_set_clock_rate(clock_type ct, u32 rate) {
    mailbox_set_clock c;
    c.tag.id = RPI_FIRMWARE_SET_CLOCK_RATE;
    c.tag.value_length = 0;
    c.tag.buffer_size = sizeof(c) - sizeof(c.tag);
    c.id = ct;
    c.rate = rate;
...
u32 mailbox_enable_qpu() {
    mailbox_qpu_state c;
    c.tag.id = RPI_FIRMWARE_SET_ENABLE_QPU;
    c.tag.value_length = 0;
    c.tag.buffer_size = sizeof(c) - sizeof(c.tag);
    c.enable = 1;
...
u32 mailbox_enable_power(u32 type) {
    mailbox_power p;
    p.tag.id = RPI_FIRMWARE_SET_POWER_STATE;
    p.tag.value_length = 0;
    p.tag.buffer_size = sizeof(p) - sizeof(p.tag);
    p.id = type;
    p.state = 0x1;
